# Definition of the training procedure for the intnet network.
...
from fast_jetclass.intnet import util as inutil
...
def main(config: dict):
    util.device_info()
    outdir = util.make_output_directory("trained_intnets", config["outdir"])
...
def build_model(config: dict, njets: int, input_size: tuple):
    ...
    model = inutil.choose_intnet(config["model_type"], config["model_hyperparams"])
    model, model_callbacks = inutil.compile_intnet(
